...
var CartPagePage = /** @class */ (function () {
    function CartPagePage(router, auth, alertController) {
        this.router = router;
        this.auth = auth;
    ...
    CartPagePage.prototype.ionViewWillEnter = function () {
        ...
        if (!JSON.parse(localStorage.getItem('cartProducts'))) {
            ...
            a.map(function (item) {
                ...
                _this.totalAmount = _this.totalAmount + parseFloat(item.variants[0].price);
            });
        }
        this.cartItems = a;
    ...
    CartPagePage.prototype.addQty = function (item, i) {
        var newamt = 0;
        var float = parseFloat(item.variants[0].price);
    ...
    CartPagePage.prototype.removeQty = function (item, i) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var newamt, float, alert_1;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
                    case 0:
                        newamt = 0;
                        float = parseFloat(item.variants[0].price);
                        if (!(item.productQuantity != 1)) return [3 /*break*/, 1];
                        this.totalAmount = this.totalAmount - (item.productQuantity * float);
                        item.productQuantity = item.productQuantity - 1;
                        newamt = item.productQuantity * float;
                        this.totalAmount = this.totalAmount + newamt;
                            ...
                            buttons: [
                                {
                                    ...
                                    text: 'Okay',
                                    handler: function () {
                                        _this.totalAmount = _this.totalAmount - float;
                                        _this.totalAmount = _this.totalAmount - (item.productQuantity * float);
                                        _this.indexFinding(i);
    ...
    CartPagePage.prototype.checkout = function () {
        localStorage.setItem('cartProducts', JSON.stringify(this.cartItems));
